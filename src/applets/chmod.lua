...
local function parse_octal(s)
  if s:match("^[0-7]+$") then return tonumber(s, 8) end
  return nil
end
...
local function apply_clause(mode, clause)
  ...
  if not op_idx then error("no operator in clause '" .. clause .. "'") end
  ...
  for b = 0, 8 do
    local v = 2 ^ b
    if (math.floor(perm_bits / v) % 2 == 1) and (math.floor(who_mask / v) % 2 == 1) then effective = effective + v end
  end
  ...
  local function bit_or(a, b)
    ...
    for k = 0, 11 do
      local v = 2 ^ k
      if (math.floor(a / v) % 2 == 1) or (math.floor(b / v) % 2 == 1) then r = r + v end
    end
    return r
  end
  local function bit_and_not(a, b)
    ...
    for k = 0, 11 do
      local v = 2 ^ k
      if (math.floor(a / v) % 2 == 1) and (math.floor(b / v) % 2 == 0) then r = r + v end
    end
    return r
  end

  if op == "+" then return bit_or(mode, effective) end
  if op == "-" then return bit_and_not(mode, effective) end
  if op == "=" then
...
local function chmod_path(path, mode_str)
  ...
  if not pipe then return false, "could not invoke chmod" end
  local out = pipe:read("*a")
  local ok, _, code = pipe:close()
  if not ok then return false, (out and out ~= "") and out:gsub("\n$", "") or ("exit " .. tostring(code)) end
  return true, nil
end
...
local function main(argv)
  ...
  local function apply(p)
    ...
    if not attr then
      if not silent then common.err_path(NAME, p, "No such file or directory") end
      rc = 1
      return
    end
    ...
    if not ok then
      if not silent then common.err_path(NAME, p, errmsg) end
      rc = 1
      return
    end
  ...
  for _, path in ipairs(paths) do
    ...
    if not attr then
      if not silent then common.err_path(NAME, path, "No such file or directory") end
      rc = 1
    else
      apply(path)
      if recursive and attr.mode == "directory" then
        common.walk(path, function(p, a)
          if p ~= path and a.mode ~= "link" then apply(p) end
        end)
      end
    end
  end
